...
%% Set program modes
mode_test = 1;	% Test mode (don't clear vars or run simulations if != 0)
...
if mode_test == 0
	...
	sim("TMS1_noPH_noORC.slx");			% 1) No preheat; no ORC
	disp('Simulating TMS2...');
	sim("TMS2_PH_noORC.slx");			% 2) Preheat; no ORC
	disp('Simulating TMS3...');
	sim("TMS3_noPH_ORC.slx");			% 3) No preheat; ORC
	disp('Simulating TMS4...');
	sim("TMS4_PH_ORC.slx");				% 4) Preheat; ORC
	disp('Simulating TMS1A...');
	sim("TMS1A_noPH_noORC_CL.slx");		% 1A) No preheat; no ORC (closed FW loop)
	disp('Simulating TMS2A...');
	sim("TMS2A_PH_noORC_CL.slx");		% 2A) Preheat; no ORC (closed FW loop)
	disp('Simulating TMS3A...');
	sim("TMS3A_noPH_ORC_CL.slx");		% 3A) No preheat; ORC (closed FW loop)
	disp('Simulating TMS4A...');
	sim("TMS4A_PH_ORC_CL.slx");			% 4A) Preheat; ORC (closed FW loop)
...
plotData(outdir_ch, y1_BoP, y2_BoP, c_BoP, 'Power consumption [W]', ...
	'grouped', [0 1E5], 'log', 'BoP', l, b_h, b_ar);
...
y2_eff = round([outTMS1A_eff; outTMS2A_eff; outTMS3A_eff; outTMS4A_eff], ...
	3, 'significant');
% Bar colours and respective legend labels
c_eff = [c.orange; c.yellow; c.green];
l     = {'Stack', 'BoP', 'Overall'};
% Plot data
plotData(outdir_ch, y1_eff, y2_eff, c_eff, 'Efficiency [%]', ...
	'grouped',[], [], 'eff', l, b_h, b_ar);
...
% Save figure
saveas(fig, [outdir_ch,'/fig_ORC.fig']);
saveas(fig, [outdir_ch,'/fig_ORC.png'], 'png');
saveas(fig, [outdir_ch,'/fig_ORC.svg'], 'svg');
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plotData(outdir,y1,y2,c,yLabel,bartype,yLim,yScale,name,l, b_h, b_ar)
	% x-axis labels
	...
	% Save figure
	saveas(fig, [outdir,'/fig_',name,'.fig']);
	saveas(fig, [outdir,'/fig_',name,'.png'], 'png');
	saveas(fig, [outdir,'/fig_',name,'.svg'], 'svg');
end
